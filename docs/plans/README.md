...
| [2026-08-14-update-checker.md](2026-08-14-update-checker.md) | 更新检查（Android 跳商店/下载页）与桌面自更新，分两步 | **第一步已实现，在 `v1.3-autoupdate` 分支待合并**：只告知不安装，客户端 `update_check.dart` / `update_service.dart` + worker `/v1/version`（端点已上线，表停在 1.2.0，合并时一并更新）。第二步桌面自更新仍是设计。macOS 已恢复构建，自 v1.2.0 起每个 tag 都发 DMG |
