...
- **da** = doc-audit:调 `doc-audit` subagent 审计文档与现实的偏差(见
  `.claude/agents/doc-audit.md`)。它只报告不改文案。
  **它是发布流程的第 1 步,不是可选项**——见「发布流程」。
...
以前这些步骤散在三个小节里,没有一处写出完整顺序——结果 `da` 永远是「有空
再做」而不是第 1 步,2026-08-23 一次审计就翻出五条已发布却没回填文档的声明
(一码多端已部署、商店条目七种、macOS DMG 已发、SDA 加密导入已上线、换机指南
已部署)。**按顺序走,不要跳。**

1. **`da`(doc-audit)** —— 发版前必跑,不是可选。它只报告不改文案;报出来的
   逐条处理完再进第 2 步。机器可判定的那部分 `tool/docs_lint.py` 在 CI 里挡,
   但**语义漂移只有它能抓**——「状态表写着待办而事情早就做完了」这类。
2. **`b`** —— bump `app/pubspec.yaml` + CHANGELOG 中英条目。
3. **本地 CI** —— `flutter analyze` 无问题 + `flutter test` 全绿,
   然后 `python3 tool/docs_lint.py <实际测试数>`。**不带数字跑它会静默跳过
   测试数检查**,等于没查。
4. **commit + push**。
5. **打 tag `v<版本>`** —— 触发 Desktop release(Windows/Linux/macOS)与
   Windows portable 两条工作流。
6. **出 Android 包** —— **必须 `flutter clean` 后构建**,play AAB + cn APK,
   拷进 `dist/` 后**按 sha256 核对**(拷贝被中断留下 0 字节文件是真实发生过的)。
   顺带跑 cn 渠道门禁(见「构建渠道」)。
7. **`/play`** —— 演练 → 确认 → `--apply`。
8. **分发** —— 见下一节「发布分发」。
9. **Play Console 点「提交审核」** —— 人工,没有 API。

## 发布分发(R2 + 站点 + 版本端点)

上一节的第 8 步。这三步漏掉任何一步都**不会报错**,只会让用户拿到旧版或死链
——`site.ts` 停在 v0.80.1 九个版本、2026-08-18 又发现它停在 v1.2.0(漏掉 1.2.1
与 1.2.2),都是这么来的。顺序执行:
...
- **正本**：仓库根 `PRIVACY.md` / `PRIVACY_ZH.md`（当前生效日期 2026-08-15，
